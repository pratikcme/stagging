...
class Api_model extends My_model {
    ...
    public function categoryCount(){
      ...
      $data['where'] = ['status!=' => '9','vendor_id' => $_POST['vendor_id']];
      $data['table'] = 'category';
    ...
    public function subcategoryCount(){
      ...
      return $subcategoryCount[0]->subcategoryCount;
    ...
    function cancle_order_quntity_reset($order_id){
       ...
       return true;
        ...
        function get_product_list($postdata) {
            ...
            if (isset($vendor_id)) {
                ...
                $data['where'] = ['p.status !=' => '9','pw.status !=' => '9','p.vendor_id'=>$vendor_id];    
                }else{
                ...
                $data['select'] = ['p.*'];
                if($defualt_product == 1){
                    $data['where'] = ['p.status !=' => '9','pw.status !=' => '9','pw.quantity >=' => '0','p.vendor_id'=>$vendor_id];    
                }else{
                $data['where'] = [
       ...
       public function userInfo($postdata){
        ...
        public function emailTemplate($user_id, $vendor_id, $o_id) {
            ...
            $data['select'] = ['id', 'vendor_id', 'user_id', 'user_address_id', 'order_no', 'isSelfPickup', 'delivery_date', 'payment_transaction_id', 'name', 'mobile', 'delivered_address', 'delivery_charge', 'total', 'dt_added', 'delivery_charge', 'order_no', 'delivery_date','total_item','total_saving','payable_amount','user_gst_number'];
         ...
         public function emailTemplate_testing($user_id, $vendor_id, $o_id) {

            $data['table'] = TABLE_ORDER;
            $data['select'] = ['id', 'vendor_id', 'user_id', 'user_address_id', 'order_no', 'isSelfPickup', 'delivery_date', 'payment_transaction_id', 'name', 'mobile', 'delivered_address', 'delivery_charge', 'total', 'dt_added', 'delivery_charge', 'order_no', 'delivery_date','total_item','total_saving','payable_amount'];
            // $data['join'] = [TABLE_ORDER_DETAILS .' as od'=>['o.id=od.order_id','LEFT']];
            $data['where'] = ['status !=' => '9', 'id' => $o_id];
            $re = $this->selectRecords($data);
            $order_id = $re[0]->id;
