...
class Api extends Apiuser_Controller {
    ...
    public function appDetails(){
        $query = $this->db->query("SELECT android_version,ios_isforce,android_isforce,ios_version from admin LIMIT 1");
        $result = $query->result();
    ...
    public function check_branch() {
        ...
        $vendorCount = $this->this_model->vendorCount();
        $response = array(
            'success' => '1', 
        ...
        if (isset($_POST['fname']) && isset($_POST['login_type'])) {
            ...
            elseif ($login_type == '1') {
                ...
                if ($facebook_query->num_rows() > 0) {
                    ...
                    $data = array('fname' => $fname, 'lname' => $lname, 'email' => $email, 'phone' => $phone, 'login_type' => $login_type, 'facebook_token_id' => $facebook_token_id, 'status' => '1', 'dt_added' => strtotime(DATE_TIME), 'dt_updated' => strtotime(DATE_TIME),);
            ...
            elseif ($login_type == '2') {
                ...
                if ($gmail_query->num_rows() > 0) {
                    ...
                    $data = array('fname' => $fname, 'lname' => $lname, 'email' => $email, 'phone' => $phone, 'login_type' => $login_type, 'gmail_token_id' => $gmail_token_id, 'status' => '1', 'dt_added' => strtotime(DATE_TIME), 'dt_updated' => strtotime(DATE_TIME),);
                    $checkuser = $this->this_model->check_register($email);
                    if ($checkuser == 1) {
                        $this->db->where('email', $email);
                        $this->db->update('user', $data);
            ...
            elseif ($login_type == '3') {
                ...
                if ($apple_query->num_rows() > 0) {
                   ...
                   $this->sendLoginResponse($login_check_result);
                    ...
                    $data = array('fname' => $fname, 'lname' => $lname, 'email' => $email, 'phone' => $phone, 'login_type' => $login_type, 'apple_token_id' => $apple_token_id, 'status' => '1', 'dt_added' => strtotime(DATE_TIME), 'dt_updated' => strtotime(DATE_TIME),);
                    $checkuser = $this->this_model->check_register($email);
                    if ($checkuser == 1) {
                        $this->db->where('email', $email);
                        $this->db->update('user', $data);
                    ...
                    $email_query = $this->db->query("SELECT * FROM user WHERE apple_token_id = '$apple_token_id'");
    ...
    public function user_login() {
        if (isset($_POST['login_type'])) {
            ...
            elseif ($login_type == '2') {
                ...
                $apple_query = $this->db->query("SELECT * FROM user WHERE apple_token_id = '$apple_token_id' AND login_type = '3'");
    ...
    public function product_detail() {
        ...
        WHERE p.status != '9' AND p.vendor_id = '$vendor_id' AND w.discount_price != '' AND w.status != '9' AND p.id = '$product_id' GROUP BY p.id ORDER BY CAST(w.discount_price AS DECIMAL(10,2))");
            ...
            if ($query->num_rows() > 0) {
                ...
                foreach ($result as $row) {
                    $product_id = $_POST['product_id'];
                    $product_weight_query = $this->db->query("SELECT * FROM product_weight WHERE status != '9' AND product_id = '$product_id'  AND vendor_id = '$vendor_id' GROUP BY id ORDER BY min(discount_price) * 1 ASC");
    ...
    function sendCustomerEmailTemplate($user_id, $vendor_id, $order_id) {
        // $order_id = 57;
