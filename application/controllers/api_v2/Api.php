...
class Api extends Apiuser_Controller {
    ...
    public function user_details() {
        ...
        if (isset($_POST['id'])) {
            ...
            $row_count = $result_count->result();
            $total_count = $row_count[0]->total;
            $my_cart(_query = $this->db->query("SELECT mc.*  FROM my_cart( as mc WHERE mc.status != '9' AND (mc.user_id = '$user_id' ) ORDER BY mc.id DESC");
            $my_cart(_result = $my_cart(_query->result();
            $my_cart(_price_query = $this->db->query("SELECT SUM(calculation_price) as total_price from my_cart( WHERE status != '9' AND user_id = '$user_id'");
            $my_cart(_price_result = $my_cart(_price_query->row_array();
            $vendorId = $my_cart(_result[0]->branch_id;
            ...
            if ($my_cart(_query->num_rows() > 0) {
                $cart_response['success'] = "1";
                $cart_response['message'] = "My cart item list";
                $cart_response["count"] = $total_count;
                $cart_response["total_price"] = number_format((float)$my_cart(_price_result['total_price'], 2, '.', '');;
                $counter = 0;
                $branch_id = $my_cart(_result[0]->branch_id;
                $total_gst = 0;
                foreach ($my_cart(_result as $row) {
            ...
            if ($query->num_rows() > 0) {
                ...
                // print_r($my_cart(_price_result);die;
                $data_user = array(
                    'id' => $result['id'], 
                    'fname' => $result['fname'], 
                    'lname' => $result['lname'], 
                ...
                foreach ($result as $row) {
                    ...
                    foreach ($product_weight_result as $pro_weight) {
                        ...
                        if (isset($_POST['user_id']) && $_POST['user_id'] != '') {
                            $my_cart(_query = $this->db->query("SELECT quantity FROM my_cart( WHERE product_id = '$product_id'AND product_weight_id = '$product_weight_id' AND branch_id = '$branch_id' AND user_id = '$user_id'");
                            $my_cart(_result = $my_cart(_query->row_array();
                        } else {
                            if (isset($_POST['device_id'])) {
                                $my_cart(_query = $this->db->query("SELECT quantity FROM my_cart( WHERE product_id = '$product_id' AND product_weight_id = '$product_weight_id' AND branch_id = '$branch_id' AND (user_id = '0' AND device_id = '$device_id')");
                                $my_cart(_result = $my_cart(_query->row_array();
                            } else {
                                $my_cart(_result = array();
                            }
                        }
                        if (empty($my_cart(_result)) {
                            $my_cart(_quantity = '0';
                        } else {
                            $my_cart(_quantity = $my_cart(_result['quantity'];
                        }
                        $img = [];
    ...
    public function checkout() {
        ...
        if (isset($_POST['payment_type']) && isset($_POST['branch_id']) && isset($_POST['time_slot_id'])) {
            if (isset($_POST['user_id'])) {
                ...
                $my_cart(_query = $this->db->query("SELECT SUM(calculation_price) as sub_total, SUM((actual_price - discount_price) * quantity) as total_savings, COUNT(*) as total_item FROM my_cart( WHERE status != '9' AND user_id = '$user_id' ");
                $my_cart(_result = $my_cart(_query->row_array();

                $sub_total = number_format((float)$my_cart(_result['sub_total'], 2, '.', '');
                $total_savings = number_format((float)$my_cart(_result['total_savings'], 2, '.', '');
                $total_item = $my_cart(_result['total_item'];
            ...
            if ($query->num_rows() > 0) {
                ...
                foreach ($result as $row) {
                    ...
                    foreach ($product_weight_result as $pro_weight) {
                        ...
                        $weight_name = $weight_result['name'];
                        if (isset($_POST['user_id'])) {
                            $my_cart(_query = $this->db->query("SELECT quantity FROM my_cart( WHERE product_weight_id = '$variant_id' AND user_id = '$user_id'");
                            $my_cart(_result = $my_cart(_query->row_array();
                        } elseif (isset($_POST['device_id']) && (!isset($_POST['user_id']) || $_POST['user_id'] == '')) {
                            $my_cart(_query = $this->db->query("SELECT quantity FROM my_cart( WHERE product_weight_id = '$variant_id' AND  device_id = '$device_id'");
                            $my_cart(_result = $my_cart(_query->row_array();
                        } else {
                            $my_cart(_result = array();
                        }
